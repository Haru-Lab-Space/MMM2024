...
class Baseline_ViT_Swin(nn.Module):
  ...
  def forward(self,img,text):
    ...
    logits = self.voting(torch.cat([logits_0.unsqueeze(-1), logits_2.unsqueeze(-1)], dim =-1)).squeeze(-1)
    return logits