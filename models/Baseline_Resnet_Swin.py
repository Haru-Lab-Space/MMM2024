...
class Baseline_Resnet_Swin(nn.Module):
  def __init__(self,image_encoder,text_encoder,text_embedding, pooling_layer):
    super(Baseline_Resnet_Swin,self).__init__()
    #image_encoder
  ...
  def forward(self,img,text):
    ...
    logits = self.voting(torch.cat([logits_1.unsqueeze(-1), logits_2.unsqueeze(-1)], dim =-1)).squeeze(-1)
    return logits